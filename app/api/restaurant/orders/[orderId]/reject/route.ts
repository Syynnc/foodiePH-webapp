...
// POST — restaurant owner rejects/cancels an order (only pending or confirmed)
export async function POST(_req: Request, { params }: { params: Promise<{ orderId: string }> }) {
    try {
        ...
        // Only allow reject on pending or confirmed orders
        ...
        if (!updated)
            return NextResponse.json({ error: "Order cannot be rejected at this stage" }, { status: 409 });

        return NextResponse.json({ ok: true });
    } catch (err) {
        console.error("[POST /api/restaurant/orders/[id]/reject]", err);
