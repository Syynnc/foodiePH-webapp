...
// POST — restaurant owner accepts a pending order → confirmed
export async function POST(_req: Request, { params }: { params: Promise<{ orderId: string }> }) {
    try {
        ...
        // Resolve which restaurant IDs belong to this owner
        ...
        if (!updated)
            return NextResponse.json({ error: "Order not available to accept" }, { status: 409 });

        return NextResponse.json({ ok: true });
    } catch (err) {
        console.error("[POST /api/restaurant/orders/[id]/accept]", err);
