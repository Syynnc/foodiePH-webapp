...
export default function DriverOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchOrder = useCallback(async () => {
    // Fetch from the driver orders list and find the matching one
  ...
  useEffect(() => {
    ...
    const iv = setInterval(fetchOrder, 12000);
    ...
    <div className="min-h-[100dvh] bg-[#FDFBF7] text-[#1a1208]">
      ...
      <div className="sticky top-0 z-30 bg-[#FDFBF7]/95 backdrop-blur-xl border-b border-[#1a1208]/[0.06]">
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center gap-4">
          <button onClick={() => router.push("/driver")} className="w-8 h-8 rounded-xl bg-[#1a1208]/[0.05] hover:bg-[#1a1208]/10 flex items-center justify-center transition-colors flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-[#1a1208]/35">Order #{order.id.slice(0, 8).toUpperCase()}</p>
          </div>
          <Link href="/" className="font-playfair text-[1rem] font-bold tracking-tight">
      ...
      <div className="max-w-2xl mx-auto px-5 py-6 space-y-6">
        ...
        {/* ── Progress tracker ── */}
        <div className="bg-white rounded-2xl border border-[#1a1208]/[0.06] p-5">
          <p className="text-[9px] uppercase tracking-[0.22em] font-bold text-[#1a1208]/35 mb-4">Delivery Progress</p>
              ...
              return (
                <div key={step.key} className="flex-1 flex flex-col items-center gap-2 relative">
                  ...
                  <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${
                    done ? "bg-[#c8783a] shadow-[0_4px_12px_rgba(200,120,58,0.3)]" : "bg-[#1a1208]/[0.06]"
                  }`}>
                    {done ? (
                  ...
                  {/* Label */}
                  <p className={`text-[10px] font-bold text-center leading-tight px-1 ${done ? "text-[#1a1208]" : "text-[#1a1208]/30"}`}>
                    {step.label}
                  </p>
                </div>
              );
          ...
          {stepIdx >= 0 && (
            <p className="text-[12px] text-[#1a1208]/45 mt-4 text-center font-light leading-relaxed">
        ...
        {/* ── Delivery address ── */}
        <div className="bg-white rounded-2xl border border-[#1a1208]/[0.06] p-5">
          <p className="text-[9px] uppercase tracking-[0.22em] font-bold text-[#1a1208]/35 mb-3">Deliver To</p>
        ...
        <div className="bg-white rounded-2xl border border-[#1a1208]/[0.06] overflow-hidden">
          <div className="px-5 pt-5 pb-3">
            <p className="text-[9px] uppercase tracking-[0.22em] font-bold text-[#1a1208]/35">{order.items.length} Items</p>
          </div>
          <div className="divide-y divide-[#1a1208]/[0.05]">
            {order.items.map((item, i) => (
              <div key={i} className="flex items-center justify-between px-5 py-3">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-[11px] font-bold text-[#1a1208]/25 w-5 tabular-nums">{item.quantity}×</span>
                  <span className="text-[13px] font-medium text-[#1a1208] truncate">{item.name}</span>
                </div>
                <span className="text-[12px] font-bold text-[#1a1208] flex-shrink-0 ml-3">
                  ₱{(item.quantity * item.unitPrice).toLocaleString()}
                </span>
              </div>
            ))}
          </div>
          <div className="border-t border-[#1a1208]/[0.06] bg-[#1a1208]/[0.015] px-5 py-4 space-y-2">
            {(() => {
              const itemsTotal = order.items.reduce((s, i) => s + i.quantity * i.unitPrice, 0);
              const diff = order.totalAmount - itemsTotal;
              return (
                <>
                  ...
                  {diff > 0 && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[12px] font-semibold text-[#1a1208] tabular-nums">₱{diff.toLocaleString()}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between pt-2 border-t border-[#1a1208]/[0.07]">
                    <span className="text-[12px] font-bold text-[#1a1208]/60 uppercase tracking-wider">Total</span>
                    <span className="font-playfair text-[1.2rem] font-bold text-[#1a1208]">₱{order.totalAmount.toLocaleString()}</span>
                  </div>
                </>
              );
            })()}
        ...
        <div className="bg-white rounded-2xl border border-[#1a1208]/[0.06] px-5 py-4 flex items-center justify-between">
          ...
          <p className="text-[13px] font-semibold text-[#1a1208]">{PAYMENT_LABELS[order.paymentMethod ?? ""] ?? order.paymentMethod}</p>
        </div>

        {/* ── Delivery proof (completed) ── */}
        {isDelivered && order.deliveryPhotoUrl && (
          <div className="bg-white rounded-2xl border border-[#10b981]/25 p-5 space-y-3">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-[#10b981]/10 flex items-center justify-center">
                <svg width="13" height="13" fill="none" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
              </div>
              <p className="text-[13px] font-bold text-[#10b981]">Delivery Completed</p>
            </div>
            <div className="relative w-full h-48 rounded-xl overflow-hidden bg-[#f5ede0]">
              <Image src={order.deliveryPhotoUrl} alt="Delivery proof" fill className="object-cover" sizes="100vw" />
            </div>
          </div>
        )}
        ...
        {!isDelivered && (
          <div className="space-y-3">
            {/* Confirm Pickup — only when preparing */}
            {order.status === "preparing" && (
              <button
                onClick={confirmPickup}
                disabled={actionLoading}
                className="w-full bg-[#c8783a] hover:bg-[#b5692e] disabled:opacity-50 text-white py-4 rounded-2xl font-bold text-[15px] shadow-[0_8px_24px_rgba(200,120,58,0.3)] transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-3"
              >
                {actionLoading ? (
            ...
            {/* Confirm Delivery — only when on_the_way */}
            {order.status === "on_the_way" && (
              <div className="space-y-3">
                <div className="bg-white rounded-2xl border border-[#1a1208]/[0.07] p-5">
                  <p className="text-[10px] uppercase tracking-[0.18em] font-bold text-[#1a1208]/35 mb-3">Delivery Proof Photo</p>
                  ...
                  {photoPreview ? (
                    ...
                    <button
                      ...
                      className="w-full border-2 border-dashed border-[#1a1208]/15 rounded-xl py-8 flex flex-col items-center gap-2 hover:border-[#c8783a]/40 hover:bg-[#c8783a]/[0.02] transition-all duration-300"
                    >
                ...
                {uploadErr && (
                  <p className="text-[12px] text-red-400 font-medium px-1">{uploadErr}</p>
                )}

                <button
                  onClick={confirmDelivery}
                  disabled={actionLoading || !photoFile}
                  className="w-full bg-[#10b981] hover:bg-[#059669] disabled:opacity-40 disabled:cursor-not-allowed text-white py-4 rounded-2xl font-bold text-[15px] shadow-[0_8px_24px_rgba(16,185,129,0.3)] transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-3"
                >
                  {actionLoading ? (
