...
function ItemDrawer({ initial, title, onSave, onCancel, saving, error }: {
    ...
    function set(k: keyof ItemForm) {
        return (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
            ...
            setForm(f => ({ ...f, [k]: val }));
            if (touched[k]) {
                setErrors(prev => ({ ...prev, [k]: validateItem({ ...form, [k]: val })[k] }));
            }
        };
    }
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a1208]/[0.07] flex-shrink-0">
                ...
                <button onClick={onCancel} className="w-7 h-7 flex items-center justify-center rounded-lg text-[#1a1208]/30 hover:text-[#1a1208]/60 hover:bg-[#1a1208]/[0.05] transition-all">
                    {Icons.close}
                </button>
            </div>
            ...
            <div className="px-6 py-4 border-t border-[#1a1208]/[0.06] flex-shrink-0 flex justify-end gap-3">
                <button type="button" onClick={onCancel} className="px-4 py-2 text-[12px] font-semibold text-[#1a1208]/45 hover:text-[#1a1208] transition-colors">
                    Cancel
                </button>
    ...
    return (
        <div className={`group relative bg-white border rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:border-[#1a1208]/15 flex flex-col ${item.isAvailable ? "border-[#1a1208]/[0.07]" : "border-[#1a1208]/[0.04] opacity-60"}`}>
            ...
            <div className="relative h-32 bg-[#F4F0EB] flex-shrink-0 overflow-hidden">
                ...
                {/* Price badge */}
                <div className="absolute bottom-2 right-2 bg-[#1a1208]/80 backdrop-blur-sm text-white font-bold text-[12px] px-2.5 py-1 rounded-full">
            ...
            <div className="p-3.5 flex flex-col flex-1 gap-2">
                <div className="flex-1">
                    ...
                    {item.description && (
                        <p className="text-[11px] text-[#1a1208]/40 mt-0.5 line-clamp-2 leading-relaxed">{item.description}</p>
                    )}
                </div>
                ...
                <div className="flex items-center gap-1.5 pt-2.5 border-t border-[#1a1208]/[0.05]">
                    <button
                        onClick={onEdit}
                        className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-[#1a1208]/45 hover:text-[#1a1208] border border-[#1a1208]/10 hover:border-[#1a1208]/25 rounded-lg py-1.5 transition-all duration-200"
                    >
                        {Icons.edit} Edit
                    </button>
                    <button
                        onClick={onToggle}
                        disabled={toggling}
                        className={`flex-1 text-[11px] font-semibold border rounded-lg py-1.5 transition-all duration-200 disabled:opacity-40 ${item.isAvailable
                    ...
                    <button
                        onClick={onDelete}
                        className="w-7 h-7 flex items-center justify-center text-[#1a1208]/20 hover:text-red-500 hover:bg-red-50 rounded-lg border border-transparent hover:border-red-100 transition-all duration-200 flex-shrink-0"
                    >
                        {Icons.trash}
                    </button>
                </div>
...
// ── Sidebar form primitives ────────────────────────────────────────────────────

const sidebarInput =
    "w-full bg-white/[0.07] border border-white/[0.10] rounded-xl px-3 py-2 text-[12px] text-white placeholder:text-white/25 outline-none focus:border-[#c8783a]/60 focus:ring-2 focus:ring-[#c8783a]/15 transition-all duration-200";

function SidebarField({ label, required, hint, children }: {
    label: string;
    required?: boolean;
    hint?: string;
...
export default function RestaurantPortal() {
    ...
    async function updateItem(f: ItemForm) {
        ...
        setSavingItem(true); setMenuError("");
        const res = await fetch(`/api/restaurant/menu/${(drawer as MenuItem).id}`, {
    ...
    const visibleItems = activeCategory === "all"
        ? menuItems
        : menuItems.filter(i => (i.category ?? "Uncategorized") === activeCategory);
    ...
    if (loading) {
        return (
            <div className="h-[100dvh] bg-[#F4F0EB] flex overflow-hidden">
                <div className="w-80 bg-white border-r border-[#1a1208]/[0.07] p-6 space-y-4 flex-shrink-0">
                    <div className="h-5 w-32 bg-[#1a1208]/[0.07] rounded-full animate-pulse" />
                    <div className="h-32 bg-[#F4F0EB] rounded-2xl animate-pulse" />
                    {[...Array(6)].map((_, i) => <div key={i} className="h-10 bg-[#1a1208]/[0.04] rounded-xl animate-pulse" style={{ animationDelay: `${i * 50}ms` }} />)}
                </div>
    ...
    return (
        <div className="h-[100dvh] bg-[#F4F0EB] flex overflow-hidden">
            ...
            {/* ── LEFT: Restaurant details panel ──────────────────────────────── */}
            <aside className="w-[300px] xl:w-[340px] flex-shrink-0 flex flex-col h-full overflow-hidden bg-[#1a1208]">
                ...
                <div className="relative h-44 flex-shrink-0 overflow-hidden bg-[#0d0905]">
                    {form.imageUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center opacity-[0.07]">
                            <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M2 7a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v1a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V7z" />
                                <path d="M4 10v9a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-9" /><path d="M10 14h4" />
                            </svg>
                        </div>
                    ...
                    {/* Restaurant name anchored to bottom */}
                    <div className="absolute bottom-0 left-0 right-0 px-5 pb-4">
                        <h1 className="font-playfair text-[1.25rem] font-bold text-white leading-snug line-clamp-2 drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]">
                            {restaurant?.name}
                        </h1>
                        {restaurant?.cuisine && (
                            <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-[#c8783a] mt-0.5">
                                {restaurant.cuisine}
                            </p>
                        )}
                    </div>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-none">
                    ...
                    <div className="px-5 pt-5 pb-2">
                        ...
                        <div className="space-y-3">
                            ...
                            <SidebarField label="Description">
                                <textarea
                                    className={`${sidebarInput} resize-none h-[72px]`}
                    ...
                    <div className="px-5 pt-3 pb-5">
                        <p className="text-[8.5px] uppercase tracking-[0.28em] font-bold text-white/25 mb-3">Media</p>
                        <SidebarField label="Cover Image URL" hint="Paste any https:// image URL">
                            <input
                                className={sidebarInput}
                                value={form.imageUrl ?? ""}
                ...
                <div className="px-5 py-4 border-t border-white/[0.06] flex-shrink-0">
                    <button
                        onClick={saveDetails}
                        disabled={savingRest || !detailsDirty}
                        className={`w-full flex items-center justify-center gap-2 rounded-xl py-3 text-[12px] font-bold uppercase tracking-[0.12em] transition-all duration-300 ${
                            detailsDirty
                                ? "bg-[#c8783a] text-white hover:bg-[#d4894b] active:scale-[0.98] shadow-[0_4px_20px_rgba(200,120,58,0.35)]"
                    ...
                    <Link
                        href="/"
                        className="flex items-center justify-center gap-1.5 mt-2.5 text-[10px] font-semibold text-white/25 hover:text-white/50 transition-colors duration-200"
                    >
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5m7-7-7 7 7 7" /></svg>
                        Back to site
                    </Link>
                </div>
            </aside>

            {/* ── RIGHT: Menu management panel ────────────────────────────────── */}
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-shrink-0 bg-[#F4F0EB] px-6 pt-5 pb-0">
                    <div className="flex items-center justify-between gap-4 mb-4">
                        <div>
                            <div className="flex items-center gap-2 text-[#1a1208]/40 mb-1">
                                {Icons.menu}
                                <span className="text-[9px] uppercase tracking-[0.22em] font-bold">Menu Management</span>
                            </div>
                            <h2 className="font-playfair text-[1.5rem] font-bold text-[#1a1208]">
                                Menu
                                <span className="ml-2.5 text-[1rem] font-normal text-[#1a1208]/30">({menuItems.length} items)</span>
                            </h2>
                        </div>
                    </div>

                    {/* Category tabs + Add button on same row */}
                    <div className="flex items-center gap-2 pb-4">
                        {/* Add Item — prominent pill on the left */}
                        <button
                            onClick={() => { setDrawer("add"); setMenuError(""); }}
                            className="flex-shrink-0 flex items-center gap-2 bg-[#c8783a] text-white rounded-full px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.12em] hover:bg-[#b5692e] active:scale-[0.97] transition-all duration-200 shadow-[0_4px_16px_rgba(200,120,58,0.35)]"
                        >
                            {Icons.plus} Add Item
                        </button>
                        ...
                        <div className="flex items-center gap-1.5 overflow-x-auto flex-1" style={{ scrollbarWidth: "none" }}>
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`flex-shrink-0 px-3.5 py-1.5 rounded-full text-[11px] font-semibold border transition-all duration-200 ${activeCategory === cat
                                        ? "bg-[#1a1208] text-white border-[#1a1208]"
                                        : "bg-white text-[#1a1208]/50 border-[#1a1208]/10 hover:border-[#1a1208]/25 hover:text-[#1a1208]/70"}`}
                                >
                        ...
                        <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                            ...
                            <div>
                                <p className="text-[13px] font-semibold text-[#1a1208]/35">
                                    {activeCategory === "all" ? "No menu items yet" : `No items in "${activeCategory}"`}
                                </p>
                                <p className="text-[11px] text-[#1a1208]/25 mt-1">Click "Add Item" to get started</p>
                            </div>
                        </div>
                    ) : (
            ...
            {/* Backdrop */}
            <div
                className={`fixed inset-0 bg-[#1a1208]/25 backdrop-blur-[2px] z-30 transition-opacity duration-300 ${drawerOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                ...
                {drawerOpen && (
                    drawer === "add" ? (
                        ...
                        <ItemDrawer
                            title="Edit Item"
                            initial={{
                                name: (drawer as MenuItem).name,
                                description: (drawer as MenuItem).description ?? "",
                                price: String((drawer as MenuItem).price),
                                imageUrl: (drawer as MenuItem).imageUrl ?? "",
                                category: (drawer as MenuItem).category ?? "",
                                isAvailable: String((drawer as MenuItem).isAvailable ?? true),
                            }}
                            onSave={updateItem}
                            onCancel={() => setDrawer(null)}
                            saving={savingItem}
